 ...
 *   3. changedPaths 含 src/scripts (渲染逻辑变更)                  -> full
 *   4. changedPaths 含 <contentDir>/ 且同时含 themes/config       -> full (保险, 两者都要)
 *   5. changedPaths 含 <contentDir>/ 下 .md (已排除 src/scripts/themes/config) -> incrementalLocal
 *   6. changedPaths 仅含 themes/config                            -> reassemble
...
export function decideStrategy(env: StrategyEnv): Strategy {
  ...
  const hasThemesOrConfig = paths.some((p) => isUnder(p, "themes") || isUnder(p, "config"));
  // 4. 内容与渲染外壳同改 -> full (保险).
  ...
  // 6. 仅 themes/config -> reassemble.
  if (paths.length > 0 && paths.every((p) => isUnder(p, "themes") || isUnder(p, "config")))
    return "reassemble";
  return "reassemble";
}
